    ...
    public void mergeReports(File reportDirectory) throws Throwable {
        ...
        for (File report : existingReports) {
            ...
            if (report.getName().equals(reportFileName)) {
            	List<String> lines = FileUtils.readLines(report);
            	String reportLines = "";
            	for(String line: lines){
            		reportLines += line;
            	}
            	
            	if(reportLines.contains("failed") && reportLines.contains("error_message")){
            		failed++;
            	} else{
            		passed++;
            	}
        ...
        ReportSummary.generateReportSummary(passed, failed, System.getProperty("environment"));
    }
